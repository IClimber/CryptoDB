...
namespace CryptoDataBase
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged
    {
        ...
        private BackgroundWorker IconChangeWorker = new BackgroundWorker();
        ...
        public MainWindow(string DatabaseFile)
        {
            ...
            IconChangeWorker.WorkerReportsProgress = true;
            IconChangeWorker.WorkerSupportsCancellation = true;
            IconChangeWorker.DoWork += new DoWorkEventHandler(ChangeIcons);
            IconChangeWorker.ProgressChanged += new ProgressChangedEventHandler(IconsChangeProgress);
            IconChangeWorker.RunWorkerCompleted += new RunWorkerCompletedEventHandler(IconsChangedCompleted);
        ...
        #region Update icons (temporary)
        private void MenuItem_Update_Icon_Click(object sender, RoutedEventArgs e)
        {
            if (IconChangeWorker.IsBusy)
            {
                IconChangeWorker.CancelAsync();
                return;
            }
            ...
            sw = Stopwatch.StartNew();
            var selectedItems = listView.SelectedItems.Cast<Element>();
            List<FileElement> imagesList = new List<FileElement>();

            GetAllElements(selectedItems, ref imagesList);

            IconChangeWorker.RunWorkerAsync(imagesList);
        }

        private void GetAllElements(IEnumerable<Element> inputList, ref List<FileElement> imagesList)
        {
            foreach (var item in inputList)
            {
                if (item.Type == ElementType.File && ImgConverter.IsImage(item.Name))
                {
                    imagesList.Add(item as FileElement);
                }

                if (item.Type == ElementType.Directory)
                {
                    GetAllElements((item as DirectoryElement).Elements, ref imagesList);
                }
            }
        }

        private void ChangeIcons(object sender, DoWorkEventArgs e)
        {
            editable = false;

            var imagesList = e.Argument as List<FileElement>;
            object lockProgress = new object();
            int progress = 0;
            int reportedProgress = 0;

            System.Threading.Tasks.Parallel.For(0, imagesList.Count, new System.Threading.Tasks.ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount }, (i, loopState) =>
            {
                ...
                var element = imagesList[i];

                lock (lockProgress)
                {
                    progress++;
                    int percentage = (int)((double)progress / imagesList.Count * 100 * 40);
                ...
                try
                {
                    ...
                    {
                        element.SaveTo(stream);
                        stream.Position = 0;

                        element.Icon = ImgConverter.GetIcon(element.Name, stream, THUMBNAIL_SIZE);
                    }
                }
                catch {}
            });
        ...
        private void IconsChangeProgress(object sender, ProgressChangedEventArgs e)
        {
            progressbar1.Value = e.ProgressPercentage / 40.0;
        }

        private void IconsChangedCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            GC.Collect();
            editable = true;
            Title = sw.ElapsedMilliseconds.ToString();
        ...
        {
            var selectedItem = (listView.SelectedItem as Element);

            DirectoryElement directory = (listView.Tag as DirectoryElement) != null ? (listView.Tag as DirectoryElement) : xdb;
            Finder f = new Finder(directory, selectedItem.Icon, selectedItem.IconPHash, ShowList);
            f.Owner = this;
            f.Show();
        }
